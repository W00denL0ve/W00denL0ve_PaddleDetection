...
def parse_args():
    ...
    parser.add_argument(
        "--output_eval",
        default="D:/dataset/work/eval_output",  # 修改默认评估输出路径
        type=str,
...
def eval():
    ...
    if FLAGS.config is None:
        ...
        FLAGS.config = os.path.normpath(FLAGS.config)
    ...
    # 确保评估输出目录存在
    os.makedirs(FLAGS.output_eval, exist_ok=True)
...
def execute_command(command, description):
    ...
    try:
        # 添加配置文件参数
        if description == "模型评估":
            config_dir = os.path.normpath(r"D:\programfiles\codes\Py_files\PaddleDetection-release-2.8.1")
            command += f" --config={os.path.join(config_dir, 'configs', 'ppq.yml')}"
            os.chdir(config_dir)
